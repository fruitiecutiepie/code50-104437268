...
def index():
    ...
    return render_template("index.html", cash=db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"]["id"]), portfolios=db.execute("SELECT symbol, shares FROM portfolios WHERE user_id = ?", session["user_id"]["id"]))
